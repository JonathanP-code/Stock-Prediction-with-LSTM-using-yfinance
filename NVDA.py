...
def get_live_NVDA_data(ticker="NVDA", period="5y", interval="1d"):
    df = yf.download(ticker, period=period, interval=interval)
    return df[['Close']]

# Fetch latest price data
symbol = "NVDA"
df = get_live_NVDA_data(symbol)
...
print(f"Last NVDA Price: ${last_price:.2f}")
...
plt.legend()
plt.title("NVDA Price Prediction (Last 30 Days + Next 10 Days)")
plt.xlabel("Date")
